...
async function callLLM(schemaKey: string, event: string): Promise<any> {
  console.log(`[LLM:${schemaKey}] Starting call for event: ${event.slice(0, 80)}...`)
  ...
  console.log(`[LLM:${schemaKey}] Response status: ${response.status} (${elapsed}ms)`)
  ...
  try {
    ...
    console.log(`[LLM:${schemaKey}] ✅ Parsed successfully. Top-level keys: ${Object.keys(parsed).join(', ')}`)
    return parsed
  } catch (parseErr) {
    console.error(`[LLM:${schemaKey}] ❌ JSON parse error: ${parseErr}`)
    console.error(`[LLM:${schemaKey}] Content preview: ${cleaned.slice(0, 500)}`)
    throw parseErr
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[trigger-analysis] Input body:`, JSON.stringify({
    ...
    console.log(`[trigger-analysis] Inserting timeline event...`)
    ...
    if (error) {
      console.error(`[trigger-analysis] Timeline insert error:`, error)
    ...
    console.log(`[trigger-analysis] All LLM calls completed in ${Date.now() - startTime}ms`)

    // Demote old snapshots
    console.log(`[trigger-analysis] Demoting old snapshots for conflict ${body.conflict_id}`)
    await supabase.from('analysis_snapshots')
      .update({ is_latest: false })
      .eq('conflict_id', body.conflict_id)

    // Insert new snapshot
    console.log(`[trigger-analysis] Inserting new snapshot...`)
    const { data: snapshot, error: snapError } = await supabase.from('analysis_snapshots').insert({
      conflict_id: body.conflict_id,
    ...
    if (snapError) {
      console.error(`[trigger-analysis] Snapshot insert error:`, snapError)
    } else {
      console.log(`[trigger-analysis] ✅ Snapshot created: ${snapshot?.id}`)
    }

    console.log(`[trigger-analysis] ✅ Complete. event_id=${event.id} snapshot_id=${snapshot?.id}`)
    ...
    console.error('[trigger-analysis] ❌ Unhandled error:', e)
    return new Response(
