...
const ConflictPage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setConflict(c.data as unknown as Conflict | null);
      setEvents((e.data || []) as unknown as TimelineEvent[]);
      setSnapshot(s.data as unknown as AnalysisSnapshot | null);
      setSnapshotHistory((sh.data || []) as unknown as Pick<AnalysisSnapshot, 'id' | 'created_at' | 'triggered_by_event_id'>[]);
      setLoading(false);
    });
  }, [id]);
  ...
  const sections = ["Timeline", "Briefing", "Causal Analysis", "Simulation", "Impact"];

  return (
    <div className="min-h-screen bg-base">
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center px-6 py-4"
        style={{ backdropFilter: "blur(20px)", background: "rgba(8,8,16,0.8)", borderBottom: "1px solid hsl(var(--border-subtle))" }}>
        ...
        <AnimatePresence>
          {bannerVisible && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
              ...
              <span className="font-mono-label text-accent-color text-xs">AI-generated analysis · Not for operational use</span>
              <button onClick={() => setBannerVisible(false)}><X className="w-4 h-4 text-og-secondary" /></button>
            </motion.div>
          )}
        </AnimatePresence>

        <ConflictHeader conflict={conflict} snapshot={snapshot} snapshotHistory={snapshotHistory} events={events} />

        <div id="section-Timeline"><TimelineView events={events} /></div>
        <div id="section-Briefing"><BriefingPanel snapshot={snapshot} actors={conflict.actors} /></div>
        <div id="section-Causal Analysis"><CausalGraphView snapshot={snapshot} /></div>
        <div id="section-Simulation"><PathwayExplorer snapshot={snapshot} /></div>
        <div id="section-Impact"><ImpactMetrics snapshot={snapshot} actors={conflict.actors} /></div>
