...
const StickyDotNav = ({ sections }: StickyDotNavProps) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      const sectionElements = sections.map((s) => document.getElementById(`section-${s}`));
  ...
  return (
    <div 
      className="fixed left-4 top-1/2 -translate-y-1/2 z-40 hidden md:flex flex-col gap-2"
        ...
        {sections.map((s, i) => (
          ...
          >
            ...
            <motion.div
              ...
              animate={{
                backgroundColor: activeSection === i ? "hsl(46,76%,59%)" : "transparent",
