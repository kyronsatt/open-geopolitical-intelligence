...
const BriefingPanel = ({ snapshot }: BriefingPanelProps) => {
  ...
  const confColor = b.confidence_level === "high" ? "text-og-green" : b.confidence_level === "medium" ? "text-accent-color" : "text-red-vivid";
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Executive Summary */}
      <GlassCard className="relative" style={{ borderLeft: "2px solid hsl(var(--accent))" } as any}>
        <span className={`absolute top-4 right-4 font-mono-label text-[10px] ${confColor}`}>
          {b.confidence_level?.toUpperCase()} CONFIDENCE
        </span>
        <p className="text-base leading-relaxed text-foreground pr-24">{b.summary}</p>
      </GlassCard>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {["usa", "iran"].map(side => {
            ...
            return (
              <div key={side} className="mb-4">
                <p className="font-mono-label text-accent-color text-[10px] mb-1">{side.toUpperCase()}</p>
                <p className="text-sm text-foreground mb-2">{data.current_posture}</p>
                {data.recent_actions?.map((a: string, i: number) => (
                  <p key={i} className="text-xs text-muted-foreground flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-og-red" />{a}
                  </p>
                ))}
              </div>
            );
        ...
        <GlassCard>
          ...
          {b.diplomatic_status?.active_channels?.map((ch: any, i: number) => {
            const dotColor = ch.status === "active" ? "bg-og-green" : ch.status === "cold" ? "bg-og-accent" : "bg-og-red";
